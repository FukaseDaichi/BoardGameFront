      ...
      <div className={styles.rollselect_background}>
        <div className={styles.turndata}>
          {props.userList.map((element: WerewolfUser, index: number) => {
            const turnFlg: boolean =
              element.handRollList && element.handRollList.length === 2;
            return (
              <div key={index}>
                <div
                  className={`${styles.imgdiv} ${turnFlg ? styles.turn : ""}`}
                >
                  <img
                    src={getIconImgUrl(element.userNo, element.userIconUrl)}
                    alt="アイコン"
                  />
                </div>
                <div className={styles.name}>{element.userName}</div>
              </div>
            );
          })}
          <div>
        ...
        <div className={styles.area}>
          {handRollFlg && (
            <span
              className={
                handRollFlg && props.user.handRollList.length === 1
                  ? ""
                  : styles.none
              }
            >
                  ...
                  return (
                    <div key={index}>
                      ...
                      <AnimationBtn
                        ...
                        viewFlg={props.user.handRollList.length === 2}
                        animeClass="heartbeat"
                      />
                      {index === 1 && props.user.userNo !== 0 && (
                        <div className={styles.memo}>渡された役職</div>
                      )}
                    </div>
                  );
                }
        ...
        <div className={styles.rollinfo}>
          ...
          <label htmlFor="rollviewcb" className={styles["check-box"]}></label>
          <div
            onClick={() => {
