          ...
          {!rushFinalFlg &&
            (haveFlg ? (
              <h1>
                CHOICE{" "}
                {
                  props.userList[userIndex].memberCardList.filter(
                    (x) => x.consumeFlg
                  ).length
                }{" "}
                / {props.userList[userIndex].memberCardList.length / 2}
              </h1>
            ) : (
              <h1>
                WAIT
              ...
              {haveFlg &&
                ...
                props.userList[userIndex].memberCardList.map((value, index) => {
                  return (
                    <div
                      key={index}
                      onClick={() => props.rush(value.no)}
                      className={value.consumeFlg ? styles.trash : ""}
                    >
                      <HoloCard
                        width={150}
                        height={210}
                        imgUrl={getMemberImgUrl(value.cardType)}
                        borderRadius={20}
                        rareFlg={value.cardType === 2 || value.cardType === -1}
                      />
                    </div>
                  );
                })}
                ...
                return (
                  <div key={index}>
                    ...
                    <div
                      ...
                      style={{
                        borderColor:
                          SystemConst.PLAYER_COLOR_LIST[member.haveUserIndex],
                      }}
                    >
                      <img
                        src={getIconImgUrl(
                          props.userList[member.haveUserIndex].userNo,
                          props.userList[member.haveUserIndex].userIconUrl
                        )}
                        alt="待機アイコン"
                      />
                    </div>
